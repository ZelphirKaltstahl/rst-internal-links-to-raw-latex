...
class RSTInternalLinksParser():
    ...
    def __init__(self):
        ...
        self.internal_link_multi_word_target = re.compile(r'[\s^]+_`(?P<link_key>[^`]+)`')
        self.internal_link_single_word_reference = re.compile(r'[\s^]+(?P<link_key>[^\s`]+)_(?!_)')
        self.internal_link_multi_word_reference = re.compile(r'[\s^]+`(?P<link_key>[^`]+)`_(?!_)')
